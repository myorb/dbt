...
const getBestProfession = async (req) => {
  ...
  if (startDate == 'Invalid Date' || endDate == 'Invalid Date')
    return {
      success: false,
      message: 'start and end date should be in valid format',
    };
  ...
  if (paidJobs.length == 0)
    return {
      success: false,
      message: 'No paid jobs performed in that period of time',
    };
...
const getBestClients = async (req) => {
  ...
  if (isNaN(countLimit))
    return { success: false, message: 'Limit is not a number' };
  ...
  const paidPerClient = paidJobs.map(function (pc) {
    return {
      ...
      fullName:
        pc.Contract.Client.firstName + ' ' + pc.Contract.Client.lastName,
